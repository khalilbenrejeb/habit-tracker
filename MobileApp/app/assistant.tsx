...
export default function Assistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: 'initial-1', text: "Yo Khalil! I'm your Daily Grind AI. What are we crushing today? 🚀", sender: 'bot' }
  ]);
  
  const scrollViewRef = useRef<ScrollView>(null);
  const API_KEY = "AIzaSyC07vlnkBPyEePkMYL-FzWZOuAoRoQtITM"; // 🔑 Paste your burner key here
  ...
  const getAIResponse = async (userText: string) => {
    ...
    try {
      ...
      const botResponse = data?.candidates?.[0]?.content?.parts?.[0]?.text || "I'm drawing a blank... try again? 🤔";

      // Fixed: Use unique timestamp ID to avoid duplicate key error
      setMessages(prev => [...prev, { id: `bot-${Date.now()}`, text: botResponse, sender: 'bot' }]);
    } catch (err) {
      // Fixed: Unique ID for error message
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'left', 'right']}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.divider }]}>
        ...
        <View style={{ marginLeft: 12 }}>
          ...
          <Text style={styles.onlineStatus}>Ready to level up</Text>
        </View>
      </View>
        ...
        >
          ...
          {loading && (
             <View style={styles.botAlign}>
                <ActivityIndicator color={colors.primary} style={{ marginLeft: 10, marginBottom: 15 }} />
             </View>
          )}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  inputRow: { flexDirection: 'row', padding: 12, paddingBottom: Platform.OS === 'ios' ? 25 : 12, alignItems: 'center', borderTopWidth: 1 },
